...
% % 위성 위치상에서의 추력 벡터 % % % % % % % % %
sat_thr_hlr = quiver3(pos(1,1), pos(1,2), pos(1,3), Thrust_vec(1,1), Thrust_vec(1,2), Thrust_vec(1,3),'c','LineWidth',2);
...
%                      'UData', vel(i,1), 'VData', vel(i,2), 'WData', vel(i,3));
    if(T_flag(i,1) == 1)
        plot3(pos(i,1),pos(i,2),pos(i,3),'g.','MarkerSize',30);
    end
    set(sat_thr_hlr, 'XData', pos(i,1), 'YData', pos(i,2), 'ZData', pos(i,3), ...
                     'UData', Thrust_vec(i,1), 'VData', Thrust_vec(i,2), 'WData', Thrust_vec(i,3));
